...
// print_r( explode(',', $row[0]['alumni_Degree']));
...
<body class="  ">
    ...
    <div class="wrapper">
        ...
        <?php
        // $insert = "SELECT * FROM `form_answers` WHERE `StudentID` = '$alumni_StudentNumber' ";
        // $user = $con->query($insert) or die($con->error);
        // $row = $user->fetch_assoc();
        // $total = $user->num_rows;

        $alumni_data = mysqli_query($con, "SELECT * FROM form_answers WHERE alumni_StudentNumber = $_SESSION[alumni_StudentNumber]");
        if (mysqli_num_rows($alumni_data) > 0) {
            while ($row = mysqli_fetch_assoc($alumni_data)) {
                // print_r($_SESSION['alumni_StudentNumber']);
                // print_r( explode(',', $row[0]['alumni_Degree']));
                // explode(',', $row[0]['alumni_DegreeDate']);
        ?>
                        ...
                        <div class="row">
                            <div class="leftcolumn">
                                <div class="card">
                                    ...
                                    <div class="rowCateg">
                                        <div class="row" style="display:inline-block">
                                            <ul style="list-style: none;">
                                                <?php foreach (explode(',', $row['alumni_Seminar']) as $sem) { ?>
                                                    <li><?php echo ($sem) ?></li>
                                                <?php } ?>
                                            </ul>
                                        </div>
                                        <div class="row" style="display:inline-block; margin-left:335px">
                                            <ul style="list-style: none;">
                                                <?php foreach (explode(',', $row['alumni_SeminarDate']) as $semD) { ?>
                                                    <li><?php echo ($semD) ?></li>
                                                <?php } ?>
                                            </ul>
                                        </div>
                            ...
                            <a href=""><input type="button" value="Edit" name="edit"></a>
                                ...
                                <div class="card">
                                    ...
                                    <p>MONTHLY EARNINGS : <?php echo $row['alumni_EmployedInitialGross']; ?></p>
