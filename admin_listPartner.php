...
if (isset($_POST['add_partner_account'])) {
    $partner_ID = $_POST['partner_ID'];
    $partner_fullName = $_POST['partner_fullName'];
    $partner_password = $_POST['partner_password'];

    $insert_query = ("INSERT INTO partner_account (PartnerID, FullName, Password) VALUES('$partner_ID', '$partner_fullName', '$partner_password')") or die('query failed');

    if ($insert_query) {

        $message[] = 'Partner Added Succesfully';
    } else {
        $message[] = 'Could Not Add The Partner';
    }
...
if (isset($_GET['delete'])) {
    $delete_id = $_GET['delete'];
    $delete_query = mysqli_query($con, "DELETE FROM partner_account WHERE PartnerID = $delete_id ") or die('query failed');
    if ($delete_query) {
        header('location:admin_listPartner.php');
...
if (isset($_POST['search'])) {
    ...
    $query = "SELECT * FROM `partner_account` WHERE CONCAT(`PartnerID`,`FullName`, `Password`) LIKE '%" . $valueToSearch . "%'";
    $search_result = filterTable($query);
} else {

    $query = "SELECT * FROM `partner_account`";
    $search_result = filterTable($query);
}
                    ...
                    <div class="col-lg-4">
                        <div class="card card-transparent card-block card-stretch card-height border-none">
                            <div class="card-body p-0 mt-lg-2 mt-0">
                                <h3 style="color: #8e3041;" class="mb-3">Partner List</h3>
                                <p style="width: 600px;" class="mb-0 mr-4">
                                    The partners list provides
                                    space to list your registered partner in the most appealing way.</p>
                    ...
                    <form action="" method="post" enctype="multipart/form-data" style=" height: 200px; margin-bottom: 50px; ">
                        <div class="details">
                            ...
                            <div class="txt_field">
                                ...
                                <input type="text" name="partner_fullName" required>
                            </div>
                            <div class="txt_field">
                                <label>Account Password</label>
                                <input type="text" name="partner_password" required>
                            </div>

                        </div>
                        <div style="position:relative;left:650px;bottom: 75px" class="row">
                            <div class="button">
                                <input type="submit" value="Add Partner Account" name="add_partner_account">
                            </div>
                        </div>
                        <!-- <div class="row">
                                    ...
                                    <?php while ($row = mysqli_fetch_array($search_result)) : ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <a href="admin_listPartner.php?delete=<?php echo $row['PartnerID']; ?>" class="delete-btn" onclick="return confirm('are your sure you want to delete this?');"> <i class="fas fa-trash"> delete </i></a>
                                            </td>
